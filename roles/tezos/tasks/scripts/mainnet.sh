...
services:
  ...
  node:
    image: $docker_image
    hostname: node
    #command: tezos-node --allow-all-rpc 0.0.0.0:8732 --rpc-addr 0.0.0.0:8732 --net-addr :$port $@
    command: tezos-node --allow-all-rpc node:8732 --rpc-addr node:8732 --private-mode --no-bootstrap-peers --connections=10 --bootstrap-threshold=1 --peer=dubnodes.tzbeta.net:9732 --peer=franodes.tzbeta.net:9732 --peer=sinnodes.tzbeta.net:9732 --peer=nrtnodes.tzbeta.net:9732 --peer=pdxnodes.tzbeta.net:9732 --net-addr :$port $@
    ports:
...
update_active_protocol_version() {
    docker run --entrypoint /bin/cat "$docker_image" \
           /usr/local/share/tezos/active_protocol_versions | /usr/bin/tail -2 > "$active_protocol_versions"
}
